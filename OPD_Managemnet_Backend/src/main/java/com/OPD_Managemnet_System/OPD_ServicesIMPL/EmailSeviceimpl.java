...
public class EmailSeviceimpl implements EmailService {
	...
	@Override
	public EmailResponce doctorEmailSend(EmailSendDTO otpDTO) {

		EmailResponce responce = new EmailResponce();

		Long otp = emailOtpGenerater.generateOTP();

		Doctor doctor = doctor_Repo.findByEmail(otpDTO.getEmail())
				.orElseThrow(() -> new ResourceNotFoundException("Invalid Doctor...."));
...
//----------------------------------Reception Email Send-------------------------------------------------

	@Override
	public EmailResponce receptionEmailSend(EmailSendDTO sendDTO) {
		
		EmailResponce responce = new EmailResponce();

		Long otp = emailOtpGenerater.generateOTP();

		Reception reception = reception_Repo.findByEmail(sendDTO.getEmail())
				.orElseThrow(() -> new ResourceNotFoundException("Invalid Patient Email...."));

		reception.setOtp(otp);
		reception_Repo.save(reception);

		try {
			SimpleMailMessage mail = new SimpleMailMessage();

			mail.setTo(sendDTO.getEmail());
			mail.setSubject("Password Recovery by Email");
			mail.setText("Your Email Otp is" + otp);

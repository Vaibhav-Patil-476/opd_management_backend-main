...
public class Doctor_ServiceIMPL implements Doctor_Service {
	...
	public Doctor save(Doctor doctor) {
		...
		try {
			return doctor_Repo.save(doctor);// repository method
		} catch (Exception ex) {
	...
	public List<Doctor> ListofDoctors() {
		try {
			return doctor_Repo.findAll();// repository method
		} catch (Exception e) {
	...
	public Doctor getBYID(int id) {
		...
		return doctor_Repo.findById(id)
				.orElseThrow(() -> new ResourceNotFoundException("Doctor dose't exists by this is" + id));
	}
